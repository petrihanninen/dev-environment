...
function createConnection(input, output) {
    if (!input && !output && process.argv.length > 2) {
        ...
        if (port) {
            ...
            let protocol = vscode_jsonrpc_1.createServerPipeTransport(pipeName);
            input = protocol[0];
            output = protocol[1];
        }
    }
        ...
        if (initializeHandler) {
            ...
            return asThenable(result).then((value) => {
                ...
                if (!capabilities.textDocumentSync) {
                    ...
                    capabilities.textDocumentSync.change = Is.number(protocolConnection.__textDocumentSync) ? protocolConnection.__textDocumentSync : protocol_1.TextDocumentSyncKind.None;
                }
                return result;
            });
        }
